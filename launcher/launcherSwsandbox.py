...
URL_LATEST	= 'http://clearlinux-sandbox.jf.intel.com/update/version/formatstaging/latest'
URL_STAGING	= 'http://clearlinux-sandbox.jf.intel.com/releases/'
IMG_HOSTNAME    = 'clearlinux-sandbox.jf.intel.com'
...
class Launcher(threading.Thread):
	def __init__(self):
		...
		self._nbuild = None
		self._url = None
		self._urlsuffix = lambda number_build : '-kvm' if str(number_build) >= 640 else ''
		self._array1=[]
		self._array2=[]
		self._array3=[]
		self._array4=[]
	...
	def arrays(self):
		...
		for n in self._array1:
			 ...
			 var3=var2.replace("['", '');
			 var4=var3.replace("']", '');
  			 var5=var4.replace("-swsandbox", '');
			 self._array3.append(var5)
		#print self._array3
...
print "Starting process launch.py for swsandbox..."
...
print "Finish process launch.py for swsandbox"
